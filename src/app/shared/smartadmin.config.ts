...
if (config.voice_command) {
    ...
    config.commands = {
        ...
        'hide navigation' : function() {
            if (jQuery( ":root" ).hasClass("container") && !jQuery( ":root" ).hasClass("menu-on-top")){
        ...
        'stop' : function() {
            window['smartSpeechRecognition'].abort();
            jQuery( ":root" ).removeClass("voice-command-active");
            jQuery.smallBox({
        ...
        'help' : function() {
            ...
            jQuery('#voiceModal').removeData('modal').modal( {
                remote: "assets/html/voice-commands.html",
                show: true } );
            if (jQuery('#speech-btn .popover').is(':visible')) {
                jQuery('#speech-btn .popover').fadeOut(250);
            }
